...
describe("real case replay - +213665267413", () => {
  it("does not allow low/accept when history has refused and cancelled outcomes", () => {
    ...
    expect(network.level === "HIGH" || network.level === "CRITICAL").toBe(true);
    expect(risk.level === "LOW").toBe(false);
    expect(risk.action === "accept").toBe(false);
  });
});
