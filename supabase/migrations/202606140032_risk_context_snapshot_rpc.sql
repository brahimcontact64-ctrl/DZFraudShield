-- Migration 032: single-shot risk context snapshot RPC for check-order hot path
...
declare
  ...
  v_identity_email_hash text;
  v_identity_address_hash text;
  ...
  v_snapshot jsonb;
begin
  -- 1) Resolve identity from hash signals (bounded to best candidate).
  with candidates as (
    select
      ci.id,
      ci.phone_hash,
      ...
      ci.updated_at,
      (
        (case when p_phone_hash is not null and ci.phone_hash = p_phone_hash then 8 else 0 end)
        + (case when p_email_hash is not null and ci.email_hash = p_email_hash then 4 else 0 end)
        + (case when p_address_hash is not null and ci.address_hash = p_address_hash then 2 else 0 end)
      ) as score
    from public.customer_identity ci
    where (
      (p_phone_hash is not null and ci.phone_hash = p_phone_hash)
      or (p_email_hash is not null and ci.email_hash = p_email_hash)
      or (p_address_hash is not null and ci.address_hash = p_address_hash)
    )
    order by score desc, ci.updated_at desc nulls last
    limit 1
  )
  select c.id, c.phone_hash, c.email_hash, c.address_hash
  into v_identity_id, v_identity_phone_hash, v_identity_email_hash, v_identity_address_hash
  from candidates c;

  -- 2) Customer reputation rollup (bounded to identity).
  if v_identity_id is not null then
    select
      coalesce(sum(cr.total_orders), 0)::int,
      coalesce(sum(cr.delivered_orders), 0)::int,
      coalesce(sum(cr.refused_orders), 0)::int,
      coalesce(sum(cr.returned_orders), 0)::int,
      coalesce(sum(cr.no_answer_count), 0)::int,
      coalesce(sum(cr.cancelled_orders), 0)::int,
      coalesce(max(cr.merchant_count), 0)::int,
      coalesce(max(cr.provider_count), 0)::int,
      max(coalesce(cr.updated_at, cr.created_at)),
      coalesce(round(avg(cr.reputation_score))::int, 50),
      coalesce(max(cr.risk_level), 'MEDIUM')
    into
      v_rep_total_orders,
  ...
  -- 3) Merchant history from indexed merchant_customer_reputation.
  ...
  -- 4) Network history from global phone reputation + reputation aggregate.
  if p_phone_hash is not null then
    select
  ...
  -- 5) Recent risk events (bounded windows + bounded reasons list).
  select
  ...
  -- 6) Ten-minute repeated activity counters (same semantics as current risk context).
  if p_phone_hash is not null then
  ...
  v_snapshot := jsonb_build_object(
    'identity', jsonb_build_object(
      ...
      'email_hash', v_identity_email_hash,
      'address_hash', v_identity_address_hash
    ),
